...
for (const section of sections) {
  ...
  execFileSync("say", ["-v", voice, "-r", rate, "-o", aiff, section.narration]);
  execFileSync("ffmpeg", [
    "-y",
    "-loglevel",
    "error",
...
Narration voice: macOS \`${voice}\`. Re-run with \`VOICE="Daniel" node scripts/build-demo-kit.mjs\` for another voice, or record your own and replace the files in \`narration/\` (keep the numbering).
`,
);
